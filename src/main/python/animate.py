...
parser.add_argument(
    "-o", "--fixed_obstacle", action="store_true", help="Animate with a fixed obstacle"
)
parser.add_argument(
...
def init():
    ...
    global board_circle
    board_circle = patches.Circle(
        (0, 0), BOARD_RADIUS, fill=False, color="black", linewidth=2
    )
    ...
    if OBSTACLE_RADIUS is not None:
        obstacle_circle = patches.Circle(
            (0, 0), OBSTACLE_RADIUS, fill=True, color="gray", alpha=0.5
        )
